...
| Remain useful when network conditions are imperfect | Partially met | `sw.js` provides a same-origin offline shell and fallback page. External GPTs, fonts, analytics, Ko-fi, and other third-party services are intentionally outside that cache boundary. |
