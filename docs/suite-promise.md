...
| Atom feed entries | 49 | The seven branch hubs plus the 42 Tool-ettes. The feed is an updates stream, not a complete mirror of the 61-page search index. | Checked-in `feed.xml`, regenerated by `scripts/generate-feed.py` |
...
`feed.xml` is checked in as a deterministic release artifact. The active
generator and its check mode are `scripts/generate-feed.py`; regenerate it
after approved catalog or feed-content changes.
