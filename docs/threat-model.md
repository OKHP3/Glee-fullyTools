...
The page-level CSP generated by `scripts/csp.py` is browser-enforced through
meta tags, with approved inline script hashes and no script `unsafe-inline`.
`scripts/check-csp.py` checks that those policies match their page contents.
Meta CSP cannot supply HTTP-only framing controls such as `frame-ancestors`.
...
`_headers` file. As a result, HTTP CSP, X-Frame-Options, X-Content-Type-Options,
