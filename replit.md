...
- **2026-05-26 — Responsive viewport QA & CSS audit (Task #1).** `assets/docs/LIVE_SITE_EVALUATION_2026-05-26.md` written. Static analysis via `scripts/responsive-audit.py` (new) across 60 pages × 8 breakpoints (320–1440px). Results: 0 P0 defects, 1 P1 fixed, 2 P2 fixed. CSS fixes in `theme.css` (GLOBAL scope): (1) `pre:not(.mermaid) { overflow-x: auto; max-width: 100%; }` — bare `<pre>` blocks now scroll on narrow viewports; (2) `flex-wrap: wrap` + `@media (max-width: 480px)` compact rule on `.site-specials` (sparkle banner). All four `scripts/*.py` validators updated to exclude `.pythonlibs` and `.cache`. `scripts/viewport-qa.py` (Playwright-based) written for future CI use. Exit: 60/60 pages 0 issues, 0 warnings, 0 broken links.
