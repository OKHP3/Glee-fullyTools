...
**The golden rule: if the component exists in the codebase, extract it.** Never rebuild from memory. You will get dimensions, colors, spacing, opacity values, and font sizes wrong. The real source code has the correct values.
...
- Specific CSS values, colors, font names, pixel spacing
...
|---|---|---|
| Rebuilding an existing component from scratch | Wrong colors, spacing, opacity  -  visual fidelity lost | Always extract first |
