...
| Moderate | WCAG 1.4.3 Contrast (Minimum) | Glee breadcrumb link hover color `#d94f63` on cream `#f6f2ee` = 3.59:1 — fails 4.5:1 for normal text. Dark mode hover already uses `#f07585` (6.04:1 ✓). AskJamie teal `#2d6f7e` re-verified: 5.1:1 on cream/paper — **passes**; original audit estimate of 3.5:1 was incorrect. | Glee breadcrumb nav on all Glee templates (light mode hover state only) | 1. Visit any Glee tool-ette page. 2. Hover over a breadcrumb link. 3. Check contrast with a tool. **Expected:** ≥ 4.5:1. **Actual (before fix):** 3.59:1. | WCAG relative luminance formula. L(#d94f63)=0.212, L(#f6f2ee)=0.893 → (0.893+0.05)/(0.212+0.05) = 3.59:1. Fix: `#b8354a` → L=0.133 → (0.893+0.05)/(0.133+0.05) = 5.16:1 ✓. | Darkened `.glee-breadcrumb-item a:hover` from `#d94f63` to `#b8354a`. | **Fixed** — Task #130 (theme.css) |
...
| Moderate | Plain-language standard | Reading level above 9th grade on four templates: Toolbox hub (~10th), Branch hub (~10th), Tool-ette detail (~10th), About (~10th). Primary causes: long sentences (>25 words), unexplained internal taxonomy (Tool-ette, branch, trunk, Function, Leaf), and portfolio engineering jargon in About. | toolbox/index.html, branch hubs, tool detail pages, about/index.html | 1. Read the hero paragraph on /toolbox/ aloud. 2. Count sentence length. | Flesch-Kincaid grade estimates from body text sampling. Longest sentences: toolbox:225–227 (39 words), about:234–240 (~35 words). | Shorten sentences over 25 words. Define GPT, Tool-ette, branch, ATS on first use. Move CI/JSON-LD detail in About to a collapsible or showcase-page reference. | **Fixed** (Task #131) — All sentences >25 words shortened across Toolbox hub, all 7 branch hubs, Resume Builder detail, and About. GPT defined "(AI assistant)" at first use per page. Tool-ette defined "(focused helper)" at first use per page. ATS defined "(applicant tracking system)" in Resume Builder. CI and JSON-LD defined in About Technical Depth card. Placeholder instruction text replaced in Identity Known. |
...
**Fixed (Task #131):** Jargon defined at first use (GPT, Tool-ette, ATS, CI, JSON-LD) across all hub and detail pages. Long-sentence reduction complete on all 9 affected pages (Toolbox hub, 7 branch hubs, Resume Builder). Placeholder instruction text removed from Identity Known.
**Remaining:** "See the butterfly's view," "Hand off elsewhere," "Explore the System" — content revisions.
